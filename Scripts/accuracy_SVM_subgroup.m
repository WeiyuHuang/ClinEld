...
network_all = temp.StrConn_all(41:74);
...
switch signal_type
    case 'FA'
        ...
        signal_all = temp.FA_all(41:74, :);
    case 'GM'
        temp = load('../RawData_matlab/GM_all.mat');
        signal_all = temp.GM_all(41:74, :);
...
elseif strcmp(signal_type, 'FA+GM')
    data = zeros(34, 2 * 116); % three features
    for i = 1:34
        network = network_all{i};
...
for C_power = 1:11
    for GAMMA_power = 1:10
        ...
        CrtPredictLabel = zeros(34, 1);
        
        for i = 1:34
            if i == 1
                CRTtrain = data(2:34, :);
                CRTtrain_label = label(2:34);
                CRTtest = data(1, :);
                CRTtest_label = label(1);
            elseif i == 34
                CRTtrain = data(1:33, :);
                CRTtrain_label = label(1:33);
                CRTtest = data(34, :);
                CRTtest_label = label(34);
            else
                CRTtrain = data([1:i-1, i+1:34], :);
                CRTtrain_label = label([1:i-1, i+1:34]);
                CRTtest = data(i, :);
                CRTtest_label = label(i);
            end % if
            
            model = svmtrain(CRTtrain_label, CRTtrain, ['-c ', num2str(C), ' -g ', num2str(GAMMA)]);
